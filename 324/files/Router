...
local function getParentId()
	...
	if not parent_id then
		while true do
			...
			local msg={action='registerRouter',id=os.getComputerID()}
...
while true do
	...
	print('Received message from Computer Id: '..tostring(id)..', '..tostring(d)..' meters away; rerouting to Parent Router: '..tostring(parent_id))
